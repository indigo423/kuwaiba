/**
 * Scripted query find physical nodes with logical ports, inside of a radius in meters 
 * with center in the given latitude and longitude.
 *
 * Pool Name: procruntime.service-feasibility
 * Name: free-nodes-radio
 * Parameters: 
 *  latitude:Double, 
 *  longitude:Double, 
...
def query = """
  MATCH (:classes{name:'GenericPhysicalNode'})<-[:EXTENDS*]-(class:classes)<-[:INSTANCE_OF]-(physicalNode:inventoryObjects)<-[:CHILD_OF*]-(logicalPort:inventoryObjects)-[:INSTANCE_OF]->(:classes)-[:EXTENDS*]->(:classes{name: 'GenericLogicalPort'})
  WHERE (:classes{name:'Antenna'})<-[:INSTANCE_OF]-(:inventoryObjects)<-[:CHILD_OF]-(logicalPort)
  AND NOT (logicalPort)-[:RELATED_TO_SPECIAL{name: 'endpointA'}]-(:inventoryObjects)
  AND NOT (logicalPort)-[:RELATED_TO_SPECIAL{name: 'endpointB'}]-(:inventoryObjects)
