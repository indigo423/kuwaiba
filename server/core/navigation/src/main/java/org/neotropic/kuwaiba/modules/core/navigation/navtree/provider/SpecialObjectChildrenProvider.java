 ...
 */
public class SpecialObjectChildrenProvider extends InventoryObjectNodeTreeProvider {
     ...
     */
    public SpecialObjectChildrenProvider(BusinessEntityManager bem, TranslationService ts) {
        this.bem = bem;
        this.ts = ts;
     ...
     */
    @Override
    public HierarchicalDataProvider buildTreeDataProvider(InventoryObjectNode rootNode, boolean incluedSelf) {
        return new AbstractBackEndHierarchicalDataProvider<InventoryObjectNode, Void>() {
            ...
            protected Stream<InventoryObjectNode> fetchChildrenFromBackEnd(HierarchicalQuery<InventoryObjectNode, Void> query) {
                ...
                try {
                    List<BusinessObjectLight> childrenQuery = new ArrayList<>();
                        ...
                        if (parent != null) {
                            addToRows(parent);
                            childrenQuery = bem.getObjectSpecialChildren(parent.getObject().getClassName(),
                                    parent.getObject().getId());
                        } else if (incluedSelf) {
                            addToRows(rootNode);
                            return Arrays.asList(rootNode).stream();
                        } else
                            childrenQuery = bem.getObjectSpecialChildren(rootNode.getClassName(), 
                                    rootNode.getId());
            ...
            public int getChildCount(HierarchicalQuery<InventoryObjectNode, Void> query) {
                try {
                    ...
                    if (parent != null)
                        return (int) bem.getObjectSpecialChildren(parent.getObject().getClassName(),
                                parent.getObject().getId()).size();
                    else if (incluedSelf)
                        return 1;
                    else
                        return (int) bem.getObjectSpecialChildren(rootNode.getClassName(),
                                rootNode.getId()).size();
            ...
            public boolean hasChildren(InventoryObjectNode node) {
                try {
                    return !bem.getObjectSpecialChildren(node.getClassName(), node.getId()).isEmpty();
                } catch (InvalidArgumentException | BusinessObjectNotFoundException | MetadataObjectNotFoundException ex) {
                    new SimpleNotification(ts.getTranslatedString("module.general.messages.error"),
                            ex.getMessage(), AbstractNotification.NotificationType.ERROR, ts).open();
                }
