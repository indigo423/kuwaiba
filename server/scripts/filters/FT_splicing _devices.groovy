 ...
 /** 
 * Retrieves splicing devices (instances of subclasses of GenericSplicingDevice and FiberSplitter) within a given parent.
...
public class %s extends Filter {
    ...
    public List<BusinessObjectLight> run(String objectId, String objectClass, HashMap<String, String> parameters, int page, int limit) throws InvalidArgumentException {
        def splicingDevices = bem.getChildrenOfClassLightRecursive(objectId, objectClass, "GenericSplicingDevice", parameters, page, limit);
        def fiberSplitters = bem.getChildrenOfClassLightRecursive(objectId, objectClass, "FiberSplitter", parameters, page, limit);
        splicingDevices.addAll(fiberSplitters);
        Collections.sort(splicingDevices);        
        return splicingDevices;
    }
}