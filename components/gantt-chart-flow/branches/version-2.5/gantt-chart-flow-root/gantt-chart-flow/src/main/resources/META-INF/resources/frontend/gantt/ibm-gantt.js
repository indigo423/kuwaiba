...
import {ganttApiLoader} from './gantt-api-loader.js';

/**
 * 
 */
class IbmGantt extends PolymerElement {
    static get template() {
        return html`
        <link href="https://www.unpkg.com/ibm-gantt-chart@0.5.3/dist/ibm-gantt-chart.css" rel="stylesheet"/>
        <style>
                html {
                    height: 100%;
                }
    ...
    ready() {
        super.ready();
        console.log("READY Gantt");
        //this.initGantt();
        new ganttApiLoader().load().then(() => {
            this.initGantt();
        });
    ...
    initGantt() {
        // Creates the chart inside the given container
        //this.chart = new GanttChart(this.$.gantt);
        var data = [
            {
                id: 'NURSES+Anne',
                name: 'Anne',
        ...
        new Gantt(this.$.gantt /* the id of the DOM element to contain the Gantt chart */, config);
    }
}

window.customElements.define('ibm-gantt', IbmGantt);